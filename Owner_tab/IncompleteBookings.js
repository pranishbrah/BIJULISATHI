...
const IncompleteBookings = () => {
  ...
  const fetchIncompleteBookings = useCallback(async () => {
    if (!stationId) {
    ...
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/owner/incomplete-bookings`,
        {
          params: { stationId },
        }
      );
      ...
      setError("Failed to load incomplete bookings. Please try again.");
    } finally {
      setIsLoading(false);
    ...
    return (
      <LinearGradient colors={["#1b44e4", "#6788f0"]} style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Incomplete Bookings</Text>
        </View>
    ...
    return (
      <LinearGradient colors={["#1b44e4", "#6788f0"]} style={styles.container}>
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading incomplete bookings...</Text>
        </View>
      </LinearGradient>
    );
  }
    ...
    <LinearGradient colors={["#1b44e4", "#6788f0"]} style={styles.container}>
      ...
      {bookings.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>No incomplete bookings found</Text>
        </View>
      ) : (
        <FlatList
          data={bookings}
